...
#xor encrypted block with next bloc and encrypt
	...
	if len(bin_input)%blockSize != 0:
		raise Exception('Input doesn\'t match block size')
...
def test_vectors():
	...
	assert_equals(calc_cipher, cipher, "test vectors")
...
def test_vectors_rfc_48():
	...
	calc_cipher = cbc_encrypt(iv, key, plain)
	assert_equals(calc_cipher, cipher, "test vectors 48 byte (3block input) from rfc)")
